...
When the sample size is large (rule of thumb: > 30 observations), the Central Limit Theorem allows us to use this test, even when the data are not normal!
...
Again, the Central Limit Theorem allows us to use this test for non-normal data when the sample size is large (rule of thumb: > 30 observations per group).
...
As before, the Central Limit Theorem allows us to use this test when the sample size is large (rule of thumb: > 30 pairs of observations).
...
for (it in seq_len(niters)) {
    g1 <- 2^rnorm(nobs) # Not normal!
    g2 <- 2^rnorm(nobs) # Not normal!
...
Here, `g1` and `g2` are drawn from the same distribution.
What should the p-value distribution look like if the t-test is working properly?
(Hint: what is the meaning of a p-value?)
...
- The SW test rejects normality when it doesn't matter 
