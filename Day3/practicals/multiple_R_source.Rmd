...
```{r}
# Put some code here!
sum(adj.pvals <= 0.05) ## ANSWER ##
which(adj.pvals <= 0.05) ## ANSWER ##
```

In practice, a better approach is to use the Holm correction.
This is slightly less conservative than the Bonferroni correction, while still controlling the FWER.
...
If I reject all tests with _q_-values below 0.05, the FDR across this set of rejected tests will be controlled (on average) at or below 5%.

```{r}
qvals <- p.adjust(all.pvals, method="BH")
# Put some code here!
sum(qvals <= 0.05) ## ANSWER ##
which(qvals <= 0.05) ## ANSWER ##
```

__Note:__ This will detect more than controlling the FWER, but at the cost of more errors.
